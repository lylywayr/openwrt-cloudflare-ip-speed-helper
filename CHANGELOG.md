...
- 缓存调度：测速地址变化、距上次全量超过 7 天、有效缓存节点不足、缓存失败率达到 30%、连续两轮速度低于基线 50% 时自动全量发现；其他轮次完整复测缓存 IP。
- 下载测速采用严格双线程分批；静态测速源预检；单 IP 失败后重试一次，双失败累计失败次数，连续 10 次淘汰，成功测速清零失败次数。
- 综合缓存增量更新：同 IP 保留综合更优端口、上限 200 条，不再每轮清空。
- 已在 iStoreOS 以最简样本实测双线程、缓存/全量切换、测速源预检、地址变化、缓存不足、失败率、低速计数和速度基线逻辑。

## 0.2.4-beta.6

- 新增全量/缓存复测调度：测速地址变化或距上次全量超过 7 天时自动全量；其余轮次完整复测综合缓存 IP。
- 下载测速改为严格双线程分批；增加静态测速源 10 MiB 预检及单 IP 失败后自动重试一次。
- 综合缓存采用增量更新、同 IP 保留更优端口、上限 200 条；不再每轮清空缓存。
- 已在 iStoreOS 以最简样本完成全量/缓存模式切换、双线程、测速源预检与地址变更触发全量的实机验证。
